...
if __name__ == "__main__":
    ...
    print(f"Ventana          : {start} a {end}  (rebalanceo semanal W-FRI)")
    print(f"Operaciones      : {len(trades)}")
    print(f"Tasa de acierto  : {hit_rate(trades):.1%}")
    print(f"Rentab. acumulada: {perf['cumulative_return']:.2%}")
    print(f"Sharpe anualizado: {perf['sharpe']:.2f}")
    print(f"Drawdown máximo  : {perf['max_drawdown']:.2%}")
    print("=" * 80)
