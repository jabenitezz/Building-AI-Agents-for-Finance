"""Chapter 6 Lab 2 — Self-Consistency with OpenAI + Financial Datasets.

Pattern:
    financial data -> N independent analyses -> parse labels -> plurality vote

This implementation does not request or expose hidden chain-of-thought. Each
path returns a concise, user-facing rationale plus a final BUY/HOLD/SELL label.
"""
...
1. Profitability and earnings quality visible in the data.
2. Growth evidence, or the lack of it.
3. Valuation evidence available in the data.
4. Key data limitations and overall risk/reward uncertainty.
...
def extract_recommendation(path: str) -> str | None:
    """Parse the terminal BUY/HOLD/SELL label robustly."""
    for line in reversed(path.strip().splitlines()):
        cleaned = line.strip().lstrip("*_`# ").upper()
        if cleaned.startswith("RECOMMENDATION:"):
            label = cleaned.removeprefix("RECOMMENDATION:").strip().strip("*_`. ")
            if label in {"BUY", "HOLD", "SELL"}:
                return label
    return None


def aggregate_recommendations(recommendations: list[str | None]) -> tuple[str, dict[str, int]]:
...
def save_report(
    ...
    path.write_text(
        f"""# Self-Consistency Equity Analysis — {ticker}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Self-Consistency stock analysis using OpenAI and Financial Datasets."
    )
    parser.add_argument(
        "--ticker",
        default="AAPL",
